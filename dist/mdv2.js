...
function escapeUrl(url) {
    let result = '';
    for (const ch of url) {
        if (_URL_ESCAPE_CHARS.has(ch))
            result += '\\';
        result += ch;
    }
    return result;
}
const _SIMPLE_MARKERS = {
    bold: ['*', '*'],
    italic: ['_', '_'],
    underline: ['__', '__'],
    strikethrough: ['~', '~'],
...
export function entitiesToMarkdownV2(text, entities) {
    if (!text)
        return '';
    ...
    function isExpandableStart(idx) {
        for (const [s, e, t] of bqRanges) {
            if (idx === s && t === 'expandable_blockquote')
                return true;
        }
        return false;
    }
    const expandableEndPositions = new Set();
    for (const [s, e, t] of bqRanges) {
        if (t === 'expandable_blockquote')
            expandableEndPositions.add(e);
    }
    const events = [];
    let seq = 0;
    ...
    for (const event of events) {
        if (!eventsByPos[event.pos])
            eventsByPos[event.pos] = [];
        eventsByPos[event.pos].push(event);
    }
    const boundarySet = new Set();
    for (const pos in eventsByPos)
        boundarySet.add(Number(pos));
    for (const pos of expandableEndPositions)
        boundarySet.add(pos);
    const boundaryPositions = Array.from(boundarySet).sort((a, b) => a - b);
    const activeCodeEntities = new Set();
    let previousEventClosedPre = false;
    const parts = [];
    let prevPy = 0;
    if (bqRanges.length > 0) {
        if (isExpandableStart(0))
            parts.push('**>');
        else if (bqAt(0) !== null)
            parts.push('>');
    }
    function emitSegment(segment, segStartPy) {
        ...
        for (let i = 0; i < segment.length; i++) {
            if (segment[i] === '\n') {
                ...
                if (isExpandableStart(nextPy))
                    parts.push('**>');
                else if (bqAt(nextPy) !== null)
                    parts.push('>');
                lineStart = i + 1;
            }
        }
    ...
    function emitTextBetween(startPy, endPy, afterPre) {
        ...
        if (segment)
            emitSegment(segment, segStartPy);
    }
    function emitTag(tag, posPy) {
...
export function splitMarkdownV2(text, entities, maxUtf16Len = 4096) {
    if (maxUtf16Len <= 0)
    ...
    while (pending.length > 0) {
        ...
        if (plainLen <= 1)
            throw new Error('A single text unit renders longer than max_utf16_len in MarkdownV2');
        ...
        if (subChunks.length === 1)
            throw new Error('Unable to split MarkdownV2 output within max_utf16_len');
