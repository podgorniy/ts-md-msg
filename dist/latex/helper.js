import { COMBINING, CombiningType, FRAC_MAP, LATEX_STYLES, LATEX_SYMBOLS, NOT_MAP, SUBSCRIPTS, SUPERSCRIPTS, } from './const.js';
export class LatexToUnicodeHelper {
    static isCombiningChar(char) {
    ...
    static translateCombining(command, text) {
    ...
    static makeSubscript(text) {
        ...
        if (sub !== null)
            return sub;
        if (text.length === 1)
            return SUBSCRIPTS[text] || `_${text}`;
        return `_(${text})`;
    }
    static tryMakeSuperscript(text) {
        if (!text)
            return '';
        let result = '';
        for (const ch of text) {
    ...
    static makeSuperscript(text) {
        ...
        if (sup !== null)
            return sup;
        if (text.length === 1)
            return SUPERSCRIPTS[text] || `^${text}`;
        return `^(${text})`;
    }
    static translateStyles(command, text) {
    ...
    static makeSqrt(index, radicand) {
        let radix = '';
        if (index === '')
            radix = '√';
        else if (index === '2')
            ...
            radix = (this.tryMakeSuperscript(index) || `(${index})`) + '√';
        return radix + this.translateCombining('\\overline', radicand);
    }
    static translateSqrt(command, option, param) {
        if (command !== '\\sqrt')
            throw new Error(`Unknown command: ${command}`);
        return this.makeSqrt(option.trim(), param.trim());
    }
    static maybeParenthesize(text) {
        const needsParens = (char) => {
            const code = char.charCodeAt(0);
    ...
    static makeFraction(numerator, denominator) {
    ...
    static translateFrac(command, numerator, denominator) {
        if (command !== '\\frac')
            throw new Error(`Unknown command: ${command}`);
        return this.makeFraction(numerator, denominator);
    }
    static translateEscape(name) {
    ...
    handleCommand(command, latex, index) {
    ...
    parseBlock(latex, start) {
        if (start >= latex.length)
            return { block: '', nextIndex: start };
        if (latex[start] !== '{') {
            if (latex[start] === '\\') {
    ...
    parseOptional(latex, start) {
        if (start >= latex.length || latex[start] !== '[')
            return { option: '', nextIndex: start };
        let level = 1;
        let pos = start + 1;
        while (pos < latex.length && level > 0) {
            if (latex[pos] === '[')
    ...
    parseSpaces(latex, start) {
        let end = start;
        while (end < latex.length && latex[end].trim() === '')
            end++;
        const substring = latex.substring(start, end);
        return { spaces: substring.includes('\n') ? '\n\n' : ' ', nextIndex: end };
    }
    parseDelimiter(latex, index) {
        if (index >= latex.length)
            return { delim: '', nextIndex: index };
    ...
    parseEnvironment(latex, index, envName) {
    ...
    renderMatrix(content, left, right, compact = false) {
