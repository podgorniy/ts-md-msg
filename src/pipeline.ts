...
import { Content, ContentTrace, ContentType, File, Photo, Text } from './content.js';
import { Segment } from './types.js';

function stripNewlinesAdjust(text: string, entities: MessageEntity[]): { text: string; entities: MessageEntity[] } {
  let leading = 0;
  ...
  if (!stripped) return { text: stripped, entities: [] };
  ...
  for (const ent of entities) {
    ...
    if (newEnd <= 0 || newOffset >= newUtf16Len) continue;
...
function sliceTextEntities(
  ...
  for (const ent of fullEntities) {
    ...
    if (entEnd <= utf16Start || entStart >= utf16End) continue;
...
function handleCodeBlock(result: Content[], seg: Segment) {
...
function handleMermaid(result: Content[], seg: Segment) {
...
export async function processMarkdown(
  ...
  const specialSegments = segments.filter(
    (s) =>
      (s.kind === 'code_block' && minFileLines > 0 && s.rawCode.split('\n').length >= minFileLines) ||
  ...
  for (const seg of specialSegments) {
    if (seg.textStart > cursorPy) {
      let { text: textChunk, entities: textEntities } = sliceTextEntities(
        fullText,
        fullEntities,
        cursorPy,
        seg.textStart,
...
export function markdownify(content: string, options?: { latexEscape?: boolean }): string {
